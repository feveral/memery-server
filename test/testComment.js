...
describe('CommentApi', function () {
    ...
    describe('#GET /api/comment/ ', async () => {
        ...
        it('should return comments related to memeId', async () => {
            ...
            expect(data[1].children).to.be.a('array')
            expect(data[1].children[0]._id).to.be.a('string')
            expect(data[1].children[0].meme_id).to.be.equal(memeId)
            expect(data[1].children[0].user_id).to.be.equal(replyUserId)
            expect(data[1].children[0].user_name).to.be.equal(replyUserName)
            expect(data[1].children[0].user_custom_id).to.be.equal(replyCustomId)
            expect(data[1].children[0].user_avatar_url).to.be.equal(replyUserAvatarUrl)
            expect(data[1].children[0].content).to.be.equal('this is a reply comment')
            expect(data[1].children[0].created_at).to.be.ISOString()
            expect(data[1].children[0].like).to.be.equal(0)
        })
    })
})